...
class POL001EC2PublicSSH(BasePolicy):
    policy_id = "POL-001"
    title = "EC2 has public SSH exposed"
    severity = "CRITICAL"
    description = "EC2 instance has port 22 open to the internet."
    ...
    def evaluate(self, snap: NormalizedSnapshot, graph: AssetGraph) -> List[PolicyResult]:
        ...
        for ec2_key, ec2 in graph.nodes.items():
            ...
            # check attached SGs for SSH open
                    ...
                    if r.get("port") == 22 and r.get("cidr") == "0.0.0.0/0":
                        results.append(
                            PolicyResult(
                                ...
                                remediation="Restrict SSH to trusted IPs or remove port 22 from public SG rules.",
